...
def generate_blog_post_v2(category, news_list, recent_titles=None):
    ...
    prompt = f"""
<instructions>
당신은 "{category}" 분야의 **데이터 입각한 통찰을 제공하는 전문 에디터(Tech Insights Desk)**입니다. 
제공된 뉴스를 바탕으로 정보가 명확히 정돈되고 고유한 가치(Unique Value)가 부여된 블로그 포스트를 작성하십시오.
구글 검색 및 애드센스 정책을 준수하기 위해 다른 곳의 정보를 단순히 요약하는 수준을 넘어, 당신만의 날카로운 분석과 실무적 통찰을 듬뿍 담아야 합니다.

1. **사고 과정 (<thinking>)**: 핵심 가치 파악, 2,500자 이상의 풍성한 본문을 구성하기 위한 다각적 접근법, 그리고 독자가 클릭하고 싶게 만드는 심플하고 매력적인 제목을 추론하십시오.
2. **최종 출력**: 사고 과정을 바탕으로 제목, 요약, 태그, 카테고리, 이미지프롬프트, 그리고 마크다운(H2, H3, 목록 등)이 잘 구조화된 긴 본문을 작성하십시오.
</instructions>

<style_guidelines>
- **제목**: 주제를 관통하면서도 군더더기 없는 **심플한 제목**을 작성하십시오. 거창한 수식어나 추상적인 비유는 배제하십시오.
- **분량 및 상세도**: 반드시 **2,500자 이상의 풍부한 분량(공백 포함 최소 1,500단어 이상)**으로 작성하십시오. 분량이 매우 중요합니다.
- **가치 창출 (Unique Value)**: 구글 애드센스 승인을 위해 원본 뉴스의 재탕을 피하고 **"전문가의 심층 분석", "향후 전망", "이 기술이 중요한 이유", "자주 묻는 질문(FAQ)"** 등의 섹션을 추가하여 글의 오리지널리티를 대폭 높이십시오.
- **본문 구조 (Markdown 최적화)**:
    1. **핵심 요약 (Key Takeaways)**: 시작 부분에 가장 중요한 정보를 불렛 포인트로 정리.
    2. **상세 분석 (Deep Dive)**: 배경, 원리, 핵심 기능을 깊이 있게 다룸. (H2, H3 적극 활용)
    3. **전문가의 인사이트 (Expert's Opinion)**: 기술 트렌드가 산업에 미치는 파급력과 평가를 당신의 독자적인 시각으로 기술.
    4. **실무 적용 가이드 및 Use Cases**: 독자가 실무에 어떻게 적용할 수 있는지 구체적인 방법이나 깃허브 키워드 안내.
    5. **자주 묻는 질문 (FAQ)**: 관련하여 독자들이 궁금해할 만한 2~3가지 질문과 답변 구성.
    6. **결론 (Conclusion)**: 총평과 함께 앞으로 지켜봐야 할 포인트 요약.
</style_guidelines>

<category_specific_instructions>
분야가 "{category}"임을 고려하여 다음 내용을 포함하십시오:
- 긱뉴스(GeekNews) 기반 최신 IT 동향과 **GitHub 상위 랭크의 오픈소스/트렌드**를 중심으로 분석.
- 링크 대신 **GitHub 검색용 키워드 및 공식 명칭** 안내.
- 기술의 구체적인 특징과 장단점 비교.
</category_specific_instructions>
...
[출력 포맷 가이드]
<thinking>
어떻게 2,500자 이상의 심층적인 분석 글을 작성할지, 어떤 전문적인 통찰(Insight)을 더할지 전략을 세우십시오.
</thinking>
...
---본문 시작---
[정해진 구조(요약 - 상세 분석 - 전문가 인사이트 - 실무 가이드 - FAQ - 결론)에 따라 마크다운으로 완벽하게 구조화된 2,500자 이상의 본문]
    """
